...
### Input 1 — Buyer Persona Spec Skill (`buyer_spec_creation_skill.md`)
...
### Input 2 — Product Knowledge (`wiki/` folder)

```
wiki/
├── index.md          ← Master catalog: all categories with summaries
...
**Navigation rule:**
- Start at `index.md` → find the category → open `items/<category>.md`
- If you need to locate a specific buyer signal or topic, check `index.md` first to find which page covers it, then open that page directly
...
Open and read `items/<category>.md` **completely** — do not skip any section.
